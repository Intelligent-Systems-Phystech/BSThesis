...
признаковых описаний в задаче классификации временных рядов\\[10mm]
    \rm\normalsize
...
\begin{center}
    ...
    \FOR{$\bh \text{ in } \mathrm{Combinations}(\bh_{\mathrm{all}})$}
        ...
        \IF{ $S_{\mathrm{best}} < S_{\mathrm{mean}}$ }
...
Physics Toolbox Suite. % TODO Сослаться на него
...
% TODO: вставить ссылку на сырые данные

\subsection{Сегментация данных}

% TODO: ссылка на статью про автоматическую чсегментацию от Мотренко.
...
\begin{center}
    ...
    \STATE $k = 0$
    \FOR{$i \in 1 \ldots l$}
    \IF{ $S(i) = \max_{j=i - F \ldots i + F}\{S(j)\}$ }
        \STATE $ \bI = (\bI, i)$
        \STATE $ k = k + 1$
    \ENDIF
    \ENDFOR
