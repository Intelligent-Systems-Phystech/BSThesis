...
\begin{frame}{Постановка задачи классификации}
    ...
    $$
    \end{block}
    \begin{block}{Определен сегмент временного ряда}% Устно: При заданной ширине сегмента и метке времени это вектор
    $$
    \bx_i  = [S(t_i), S(t_i - d), S(t_i - 2d), \ldots, S(t_i - (n - 1)d)]^\intercal,
\;\; \bx_i \in X \equiv \R^n.
    $$
    \end{block}
        Задана выборка $\mathfrak{D} = \{ (\bx_i, y_i) \}_{i=1}^l, \;\;\; y_i \in \{1, 2, \ldots K\}$.
